    ...
    <section id="projects" className="section">
      <div className="wrap">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {state === 'ready'
            ? photos.map((photo, i) => (
                <Reveal key={photo.name} variant="scale" delay={Math.min(i, 5) * 80}>
                  <button
                    type="button"
                    onClick={() => setLightbox(photo)}
                    className="group block w-full overflow-hidden rounded-xl border border-paper-line bg-surface text-left shadow-card transition-shadow duration-200 hover:shadow-card-hover"
                  >
            ...
            : [0, 1, 2, 3, 4, 5].map((i) => (
                <Reveal key={i} variant="scale" delay={Math.min(i, 5) * 80}>
                  <PlaceholderTile />
                </Reveal>
              ))}
        </div>
